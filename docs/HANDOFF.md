...
> 4. **协作转写遗留项系统性实施（主人指示）**：`observatory/yuyi-transcribe.mjs`——读 Hub 库（messages / delivered_index / message_events / would_deny_events）+ yuyi-agent/events.jsonl → `collab.*` 事件；按 agent.json 自身份判方向、只搬与自身相关行；检查点幂等；身份三元组只取 Hub 权威回填、词表外 gate 动作跳过不编造、**正文 text 永不进事件**。omp 容器：compose 白名单挂 `observatory/`（ro）+ entrypoint 内建 15s 转写循环（Yuyi 未接入则显式停用）。验收 `contracts/yuyi-transcribe-e2e.mjs` 14 断言全过（真实 schema 夹具）。**容器生效需重建**：`docker compose up -d --build`（挂载与 entrypoint 变更）。