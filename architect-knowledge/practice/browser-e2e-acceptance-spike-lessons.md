...
**目标措辞敏感（2026-09-26 实测新增）**：DONE 判断对目标句措辞强敏感——同一任务同一页面，
「Stop when the Casa Flora detail page is visible」9 步 DONE，改成代词化「Stop when its detail page
is visible」则永不 DONE（25 步打满，导航虽每次都正确）；根因是 v10s 按特定句式风格微调，代词化
改写掉出训练分布。**验收目标须贴近套件基线句式**（导航式、点名具体对象，勿用代词指代页面/元素）；
拿不准的措辞先跑一条 fixture 任务冒烟验证再上正式验收。
