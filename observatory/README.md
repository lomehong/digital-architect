...
├── yuyi-transcribe.mjs         协作消息实例侧转写（~/.yuyi → collab.* 事件；检查点幂等；正文永不入库）
├── alert-rules.yml            告警规则 v1（5 条，支持抑制窗口）
├── data-roots.yml             治理地址簿（实例 → 台账根）
├── public/index.html          看板单页（原生 JS，无构建，8 视图）
├── contracts/                 事件契约校验器 + 渲染烟测 + 镜像/转写/governor e2e + 审批代办契约
...
- **Model B / C3 ✅ 完成（主人 2026-09-12 已确认）**：governor 远端执行面——指令单（管理员令牌 + HMAC 签名 + 幂等单号 + 双侧白名单 + 全量回执留痕）；宿主清单=先本机 dsh + omp 容器；走查见 `docs/designs/2026-09-12-平台独立部署演进-C3-走查记录.md`
- **Model B / C4 ✅ 完成（主人 2026-09-12 已确认）**：云主机迁移手册（凭据清单 / 部署步骤 / 验证与负测清单 / 镜像分叉收敛 / systemd / 回滚）——`docs/designs/2026-09-12-平台独立部署演进-C4-云主机迁移手册.md`
- **Model B 结案（2026-09-12）**：四期全部落定；现役平台已重启载入 C2/C3 能力（随仓运行语义不变：brain mode=repo、发单门待配 `GOV_SIGN_KEY` 后启用）；云主机实际切换按 C4 手册待窗口
