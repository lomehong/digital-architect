...
**接入工具**：`observatory/heartbeat.mjs`（零依赖）——通用实例心跳，**只刷新** `status`/`lastSeenAt`/`heartbeatIntervalSec`，保留实例自身声明的 `systems`/`capabilities`/`host`（心跳不覆盖身份声明）；可选 `--yufu-url` + `YUFU_CREDENTIAL` 上报身份自验证据，**未配置则不发事件**（不接受凭空证据）。omp 容器另有 `entrypoint.sh` 内置循环。
...
**治理操作可信性防线**（2026-09-12 完成加固并经**主人追认**；起因：本会话两次在负测中误对生产台账执行 confirm，均已回滚并留痕）：
...
| **上报认证（受保护模式）** | 绑定非回环或 `--require-token` 时：上报须 `Authorization: Bearer <御符token>`（平台调 `yufu_verify` 验证，agentId 以 `verifiedAs` 平台注记落盘，缓存 10min 仅内存）；读/治理端点要求 `x-obs-admin` 管理员令牌；缺配置拒绝启动 | 多宿主上报架构的信任基础——「谁在上报」由御符权威回答，而非实例自报 |

> 多宿主架构语义（2026-09-12 主人拍板实施认证后明确）：**事件契约是唯一真实数据面**，远程实例一律经认证 HTTP 上报；文件约定与 agent.db / yuyi 本机摄取是「同宿主优化」，仅覆盖与平台同机的实例；跨主机治理需实例侧远端执行面（仍登记）。
> **部署前提（显式化，2026-09-12）**：当前平台**随大脑仓部署**——平台代码、知识治理、配置、数据根都在仓内，仓在哪平台在哪。大脑仓多副本/平台独立部署的演进方案见 `docs/designs/2026-09-12-平台独立部署演进-技术方案.md`（Model B：打包解耦 / 配置迁数据根 / 大脑仓 git 镜像 / governor 远端执行面）。**实施进展（2026-09-12，C1/C2 主人已确认落定）**：C1 打包与配置解耦完成（baf41a8）；C2 大脑仓 git 镜像完成（86baa10）——知识面独立：fetch/ff-only 只读同步，知识升级=镜像内 commit+push，冲突拒写呈报（`--brain` 启用，回归 `observatory/contracts/brain-mirror-e2e.mjs`）。同场决策：部署形态=**云主机**（C4 输入）；C3 governor 宿主清单=先本机 dsh+omp 容器（C3 已放行）。评审结论与走查记录：`docs/designs/2026-09-12-平台独立部署演进-{评审结论,走查记录}.md`。

> 残余风险（登记）：平台绑定 127.0.0.1 且**无认证**——防线防的是「自动化/负测误写」，不防本机恶意进程。
> **升级触发条件**：若平台改为监听**非回环地址**，或引入**多用户/远程访问**，则「本地令牌或复用御符身份」从登记项升级为**前置必修**。
> **事故复盘**：`docs/designs/2026-09-12-治理写操作误用-事故复盘.md`（完整时间线 / 分层根因 / 影响面 / 加固实测 / 残余风险）。
...
| **知识库只读摄取** | 三期+ | ✅ 完成并实测 | 五类目录 frontmatter 轻解析（`loadKnowledgeBase`）：38 条、分类/状态分布、最近更新排序；修复「知识视图只有事件没有知识库本体」的缺口 |
...
| 治理可信性 | 操作者显式 + 真实台账确认 + 负测隔离 + 回滚留痕（§7.4） | 架构师会话自主加固，**主人 2026-09-12 追认** |
