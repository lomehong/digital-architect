...
import { lintIssue, lintKnowledge, relativePathCandidates } from "./lint.js";
export const KNOWLEDGE_DIRS = ['meta', 'principle', 'scenario', 'practice', 'reference'];
const ENTRY_EXT_RE = /\.md$/i;
/** ── 条目 frontmatter 解析（受限格式：缩进 0 的 `key: value` + 单层嵌套 `key:` 下两空格 `sub: value`）── */
export function parseEntry(content, relPath, issues) {
...
export function parseQueue(content, issues) {
    const rows = [];
    let inRow = false;
    for (const raw of content.replace(/^\uFEFF/, '').split(/\r?\n/)) {
        ...
        if (line.startsWith('- ')) {
            ...
            if (m) {
                rows.push({ file: m[1].trim() });
                inRow = true;
            }
            else
                issues.push(lintIssue('R8', 'review-queue.yaml', `无法解析的行组首行：${JSON.stringify(raw)}`));
            continue;
        }
        if (inRow && /^[\w-]+:\s*/.test(line))
            continue;
...
export function parseIndex(content, relPath, issues) {
    const rows = [];
...
export function collectKnowledgeSnapshot(rootArg, cwd = process.cwd()) {
    ...
    for (const dir of KNOWLEDGE_DIRS) {
        ...
        for (const name of readdirSync(dirPath)) {
            if (!ENTRY_EXT_RE.test(name) || name === 'index.md')
                continue;
            ...
            if (fields !== null)
                entries.push({ path: rel, fields, body: content });
        }
    }
...
export function makeRefExists(root) {
    const parentOfRoot = dirname(root);
    return (entryPath, ref) => {
...
export function lintKnowledgeAt(rootArg, cwd = process.cwd()) {
    ...
    for (const i of collected.issues)
