...
> **2026-09-12 补记三（Model B / C3-C4 收官）**
>
...
> 4. **协作转写遗留项系统性实施（主人指示，2026-09-13 拓扑实测更正设计）**：`observatory/yuyi-transcribe.mjs`——读 Hub 镜像（hub/inbox.db 四表 + yuyi-agent/events.jsonl）→ `collab.*` 事件；按自身份判方向、只搬与自身相关行；检查点幂等；身份三元组只取 Hub 权威回填、词表外 gate 动作跳过不编造、**正文 text 永不进事件**。**拓扑更正**：Hub 在云（hub.qianji.io），`hub/inbox.db` 是 Hub 同机侧守护进程的本地镜像，omp 容器为纯客户端本地无镜像——转写在**镜像同机侧**执行：平台内建循环（data-roots.yml `agentId` 映射逐实例归属，映射非空自动启用，`--no-yuyi-transcribe` 关闭）；容器 entrypoint 仅在本地真有镜像时才启动循环（单条诚实说明，不再空转）。**映射（有据）**：omp-ops-pi-01 → `5747e738…`（宿主 agent.json "omp" 条目）；dsh-architect-01 → `88efe2d1…`（真实御符自验事件）。验收 `contracts/yuyi-transcribe-e2e.mjs` 16 断言全过（真实 schema 夹具，含 --self 路径与归属过滤）+ 真实镜像实测。**主人已重启平台，实测生效**（2026-09-13）：health sources.yuyiTranscribe=ok、mapped=2、totalEmitted=2——两条真实闸门事件（test-agent terminated）已按实例归属转写上屏。