...
> **部署前提（显式化，2026-09-12）**：当前平台**随大脑仓部署**——平台代码、知识治理、配置、数据根都在仓内，仓在哪平台在哪。大脑仓多副本/平台独立部署的演进方案见 `docs/designs/2026-09-12-平台独立部署演进-技术方案.md`（Model B：打包解耦 / 配置迁数据根 / 大脑仓 git 镜像 / governor 远端执行面）。**实施进展（2026-09-12）**：C1 打包与配置解耦已完成（baf41a8）；C2 大脑仓 git 镜像已实施——知识面独立：fetch/ff-only 只读同步，知识升级=镜像内 commit+push，冲突拒写呈报（`--brain` 启用，回归 `observatory/contracts/brain-mirror-e2e.mjs`）；C3 governor / C4 迁移手册待主人决策门。评审结论与走查记录：`docs/designs/2026-09-12-平台独立部署演进-{评审结论,走查记录}.md`。
