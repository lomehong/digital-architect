...
export function checkDigest(input) {
    const missing = [];
    const coverage = [];
    ...
    if (req === '')
        missing.push('补需求原文');
    if (!hasDo)
        missing.push('把需求条目逐项归属为「做/不做/待确认」（至少要有「做」）');
    ...
    if (!hasSystems)
        missing.push('列出涉及的服务/仓库/上下游（若无也需显式声明空集原因）');
    ...
    if (evidences.length === 0)
        missing.push('登记关键结论及其证据来源');
    if (badEvidence.length > 0)
        missing.push('为每条关键结论补证据来源（当前行为以代码为准）');
    coverage.push({
        item: '④ 风险覆盖（兼容/异常/缓存/MQ/状态/安全）',
        ok: input.risksChecked === true,
        note: input.risksChecked === true ? '已逐类检查' : '风险检查未声明完成',
    });
    if (input.risksChecked !== true)
        missing.push('逐类过风险清单后置 risksChecked=true');
