...
export function countEmptyCells(text) {
    let count = 0;
...
export function countUnfilledPlaceholders(text) {
...
/** 检查五问速答表。 */
export function checkFiveQuestions(md) {
    const issues = [];
...
/** 对单个维度区段打分（0-10）。 */
function scoreDimension(sec, key, title) {
    const maxScore = 10;
    if (sec === undefined) {
...
export function checkDesign(md) {
    ...
    for (const d of dimensions)
        issues.push(...d.issues.map(i => `【${d.title}】${i}`));
...
export function renderReviewSkeleton(title, result) {
