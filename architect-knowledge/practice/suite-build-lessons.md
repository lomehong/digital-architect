...
| **Agent 的「常驻」写成计划任务/隐藏窗口 = 木马特征**（严重·本机） | 为让平台脱离开发会话常驻，注册了登录自启计划任务（`%LOCALAPPDATA%` 内 node.exe 周期写文件）并用 `Start-Process -WindowStyle Hidden` 启动；Defender 判 `Trojan:Win32/Bearfoos.A!ml`（检测落到 **dsh-desktop.exe 本体**+快捷方式+卸载项）与 `Trojan:Win32/PowhidSubExec.B`，**进程树被杀、宿主应用被挡**（2026-09-12 本机实测） | **禁止**为「常驻」创建登录自启计划任务、**禁止**隐藏窗口启动；常驻能力内建进自身进程（如平台 `--heartbeat`）；确需自启必须先与主人确认并配置杀软排除项。行为检测杀软看的是**行为链**（持久化+AppData 可执行+快捷方式+卸载项），不看代码意图 |
